...
class ChatFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        userAdapter = UserAdapter(mutableListOf(), requireContext())
        setupRecyclerView()
        setupChatListener()

        binding.sendMessage.setOnClickListener {
            if (binding.messageEd.text.toString().isNotEmpty()) {
    ...
    private fun sendMessage(text: String?) {
        val database = FirebaseDatabase.getInstance().reference.child("login").child("email").child(FirebaseAuth.getInstance().currentUser?.uid.toString())
        ...
        database.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (picture.isNotEmpty() && picture.isNotBlank()) {
                    ...
                    chatMessages.child("message").setValue(text)
                    chatMessages.child("fullname").setValue(fullname)
